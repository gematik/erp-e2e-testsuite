...
public class OrganisationFuzzImpl implements FhirResourceFuzz<Organization> {
    ...
    public Organization generateRandom() {
        ...
        fuzzerContext.getTypeFuzzerFor(Narrative.class).ifPresent(tf -> org.setText(tf.generateRandom()));
        fuzzerContext.getTypeFuzzerFor(Extension.class).ifPresent(tf -> org.setExtension(List.of(tf.generateRandom())));
        fuzzerContext.getTypeFuzzerFor(Identifier.class).ifPresent(tf -> org.setIdentifier(List.of(tf.generateRandom())));
        org.setName(fuzzerContext.getStringFuzz().generateRandom(15));
        fuzzerContext.getTypeFuzzerFor(ContactPoint.class).ifPresent(tf -> org.setTelecom(List.of(tf.generateRandom())));
        fuzzerContext.getTypeFuzzerFor(Address.class).ifPresent(tf -> org.setAddress(List.of(tf.generateRandom())));
        return org;
    }
    ...
    private void telecomFuzz(Organization o) {
        val contPointFuzz = fuzzerContext.getTypeFuzzerFor(ContactPoint.class, () -> new ContactPointFuzzImpl(fuzzerContext));
        ...
        if (cp == null) {
            ...
            fuzzerContext.addLog(new FuzzOperationResult<>("fuzzed Telecom in Organization",
                    cp, o.hasTelecom() ? o.getTelecom().stream().map(Object::toString).collect(Collectors.joining(", ")) : null));
        }
    }
    ...
    private void metaFuzz(Organization o) {
        ...
        fuzzerContext.getTypeFuzzerFor(Meta.class).ifPresent(tf -> tf.fuzz(o::hasMeta, o::getMeta, o::setMeta));
    ...
    private void identifyFuzz(Organization o) {
        if (!o.hasIdentifier()) {
            fuzzerContext.getTypeFuzzerFor(Identifier.class).ifPresent(tf -> o.setIdentifier(List.of(tf.generateRandom())));
            fuzzerContext.addLog(new FuzzOperationResult<>("set Identifier in Organization", null, o.hasIdentifier() ? o.getIdentifier() : null));
        } else {
            val org = o.getIdentifierFirstRep().copy();
            val listFuzz = new ListFuzzerImpl<>(fuzzerContext, fuzzerContext.getTypeFuzzerFor(Identifier.class, () -> new IdentifierFuzzerImpl(fuzzerContext)));
    ...
    private void textFuzz(Organization o) {
        ...
        fuzzerContext.getTypeFuzzerFor(Narrative.class).ifPresent(tf -> tf.fuzz(o::hasText, o::getText, o::setText));
    ...
    private void extensionFuzz(Organization o) {
        ...
        if (!o.hasExtension()) {
            ...
            fuzzerContext.addLog(new FuzzOperationResult<>("fuzz Extension in Organization", org, o.hasExtension() ? o.getExtension() : null));
        }
    }
    ...
    private void addressFuzz(Organization o) {
        ...
        if (!o.hasAddress()) {
            ...
            fuzzerContext.addLog(new FuzzOperationResult<>("fuzz Address in Organization", org, o.hasAddress() ? o.getAddress() : null));
        }
    }
