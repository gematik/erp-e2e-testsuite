...
public class CoverageFuzzImpl implements FhirResourceFuzz<Coverage> {
    ...
    public Coverage generateRandom() {
        ...
        fuzzerContext.getTypeFuzzerFor(Extension.class).ifPresent(tf -> cov.addExtension(tf.generateRandom()));
        cov.setStatus(fuzzerContext.getRandomOneOfClass(Coverage.CoverageStatus.class, Coverage.CoverageStatus.NULL));
        fuzzerContext.getTypeFuzzerFor(CodeableConcept.class).ifPresent(tf -> cov.setType(tf.generateRandom()));
        fuzzerContext.getTypeFuzzerFor(Reference.class).ifPresent(tf -> cov.setBeneficiary(tf.generateRandom()));
        fuzzerContext.getTypeFuzzerFor(Period.class).ifPresent(tf -> cov.setPeriod(tf.generateRandom()));
        fuzzerContext.getTypeFuzzerFor(Reference.class).ifPresent(tf -> cov.setPayor(List.of(tf.generateRandom())));
        return cov;
    }
    ...
    private void periodFuzz(Coverage c) {
        if (!c.hasPeriod()) {
            ...
            val org = c.getPeriod();
            fuzzerContext.getTypeFuzzerFor(Period.class).ifPresent(tf -> tf.fuzz(c::getPeriod, c::setPeriod));
    ...
    private void beneficiarityFuzz(Coverage c) {
        ...
        fuzzerContext.getTypeFuzzerFor(Reference.class).ifPresent(tf -> tf.fuzz(c::hasBeneficiary, c::getBeneficiary, c::setBeneficiary));
