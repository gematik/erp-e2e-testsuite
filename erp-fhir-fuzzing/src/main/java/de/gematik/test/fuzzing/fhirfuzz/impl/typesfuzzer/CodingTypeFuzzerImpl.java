...
public class CodingTypeFuzzerImpl implements FhirTypeFuzz<Coding> {
  ...
  private void fuzzId(Coding c) {
    ...
    if (!c.hasId()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>("fuzz Id in Coding", orgCode, c.hasId() ? c.getId() : null));
    }
  }
  ...
  private void fuzzUserSel(Coding c) {
    if (!c.hasUserSelected()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>("set UserSelect in Coding", null, newUserSelect));
    } else {
      val userSelected = c.getUserSelected();
      val newUserSelec = !userSelected;
      c.setUserSelected(newUserSelec);
      fuzzerContext.addLog(
          new FuzzOperationResult<>("fuzz UserSelect in Coding", userSelected, newUserSelec));
    }
  }
