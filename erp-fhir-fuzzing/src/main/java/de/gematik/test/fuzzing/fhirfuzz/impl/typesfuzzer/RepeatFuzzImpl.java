...
public class RepeatFuzzImpl implements BaseFuzzer<Timing.TimingRepeatComponent> {
    ...
    private void fuzzId(Timing.TimingRepeatComponent t) {
        if (!t.hasId()) {
            ...
            fuzzerContext.addLog(new FuzzOperationResult<>("fuzzRepeat.id:", org, t.hasId() ? t.getId() : null));
        }
    }
    ...
    private void fuzzDurationMax(Timing.TimingRepeatComponent t) {
        if (!t.hasDurationMax()) {
            ...
            fuzzerContext.addLog(new FuzzOperationResult<>("generate Repeat.DurationMax ", null, duration));
        } else {
    ...
    private void fuzzFrequency(Timing.TimingRepeatComponent t) {
        if (!t.hasFrequency()) {
            ...
            fuzzerContext.addLog(new FuzzOperationResult<>("generate Repeat.Frequency ", null, frequency));
        } else {
    ...
    private void fuzzFrequencyMax(Timing.TimingRepeatComponent t) {
        if (!t.hasFrequencyMax()) {
            ...
            fuzzerContext.addLog(new FuzzOperationResult<>("generate Repeat.FrequencyMax ", null, frequency));
        } else {
            if (fuzzerContext.shouldFuzz(t.getFrequencyMax())) {
                ...
                t.setFrequencyMax(frequency);
                fuzzerContext.addLog(new FuzzOperationResult<>("fuzz Repeat.FrequencyMax ", org, frequency));
            }
        }
    }
    ...
    private void fuzzPeriodUnit(Timing.TimingRepeatComponent t) {
        if (!t.hasPeriodUnit()) {
            val period = fuzzerContext.getRandomOneOfClass(Timing.UnitsOfTime.class, Timing.UnitsOfTime.NULL);
            ...
            if (fuzzerContext.shouldFuzz(t.getPeriodUnit())) {
                ...
                val org = t.getPeriodUnit();
                val period = fuzzerContext.getRandomOneOfClass(Timing.UnitsOfTime.class, List.of(Timing.UnitsOfTime.NULL, t.getPeriodUnit()));
                t.setPeriodUnit(period);
    ...
    private void fuzzTimeOfDay(Timing.TimingRepeatComponent t) {
        if (!t.hasTimeOfDay()) {
            ...
            t.setTimeOfDay(time);
            fuzzerContext.addLog(new FuzzOperationResult<>("generate Repeat.TimeOfDay ", null, time.get(0)));
        } else {
            if (fuzzerContext.shouldFuzz(t.getPeriodUnit())) {
                ...
                val org = t.getTimeOfDay();
                org.forEach(timeType -> timeType.setValue(fuzzerContext.getStringFuzz().fuzz(timeType.getValue())));
                val time = List.of(new TimeType(fuzzerContext.getStringFuzz().generateRandom(5)), new TimeType(fuzzerContext.getStringFuzz().generateRandom(5)), new TimeType(fuzzerContext.getStringFuzz().generateRandom(5)));
                t.setTimeOfDay(time);
    ...
    public Timing.TimingRepeatComponent generateRandom() {
        ...
        repeat.setCount(fuzzerContext.getRandom().nextInt())
                ...
                .setDurationUnit(fuzzerContext.getRandomOneOfClass(Timing.UnitsOfTime.class, Timing.UnitsOfTime.NULL))
                ...
                .setPeriodUnit(fuzzerContext.getRandomOneOfClass(Timing.UnitsOfTime.class, Timing.UnitsOfTime.NULL))
