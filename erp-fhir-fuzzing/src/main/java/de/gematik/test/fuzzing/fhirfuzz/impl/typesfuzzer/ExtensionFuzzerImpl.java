...
public class ExtensionFuzzerImpl implements FhirTypeFuzz<Extension> {
  ...
  private void fuzzUrl(Extension ex) {
    if (!ex.hasUrl()) {
      ...
      fuzzerContext
          .getUrlFuzz()
          .fuzz(ex::getUrl, o -> ex.setUrl(fuzzerContext.getUrlFuzz().fuzz(o)));
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "fuzzes URI in Extension ", url, ex.hasUrl() ? ex.getUrl() : null));
    }
  }
  ...
  private void fuzzId(Extension ex) {
    ...
    fuzzerContext.addLog(
        new FuzzOperationResult<>(
            "Changes Id in Extension ", value, ex.hasId() ? ex.getId() : null));
  }
  ...
  private void fuzzExt(Extension ex) {
    if (!ex.hasExtension()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>("Changes Extension in Extension ", null, List.of(ex2)));
    } else {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              ...
              value,
              ex.hasUrlElement() ? ex.getUrlElement() : null));
    }
  }
  ...
  private void fuzzValue(Extension ex) {
    ...
    ex.setExtension(null);
    fuzzerContext.addLog(
        new FuzzOperationResult<>("set Value in Extension and set Extension to null", null, value));
  }
    ...
    if (ex.hasType("CodingType")) {
      for (val e : ex.getExtension()) {
        if (e.getValue() instanceof StringType stringType) {
          ...
          fuzzerContext.addLog(
              new FuzzOperationResult<>("Changes DateType in Extension ", org, dateType));
          ...
          codingTypeFuzzer.ifPresent(
              tf ->
                  tf.fuzz(
                      () -> (Coding) codeableConcept.getCoding(),
                      o -> codeableConcept.setCoding(List.of(o))));
        }
      }
    }
  }
