...
public class PractitionerFuzzImpl implements FhirResourceFuzz<Practitioner> {
    ...
    public Practitioner generateRandom() {
        ...
        fuzzerContext.getTypeFuzzerFor(Identifier.class).ifPresent(tf -> prac.setIdentifier(List.of(tf.generateRandom())));
        fuzzerContext.getTypeFuzzerFor(HumanName.class).ifPresent(tf -> prac.setName(List.of(tf.generateRandom())));
        return prac;
    }
    ...
    private void identifyFuzz(Practitioner p) {
        ...
        if (!p.hasIdentifier()) {
            ...
            fuzzerContext.addLog(new FuzzOperationResult<>("set Identifier in Practitioner", null, ident));
        } else {
    ...
    private void birthdayFuzz(Practitioner p) {
        ...
        if (!p.hasBirthDate()) {
            ...
            fuzzerContext.addLog(new FuzzOperationResult<>("set new BirthDate in Practitioner", null, birth));
        } else {
    ...
    private void addressFuzz(Practitioner p) {
        ...
        if (!p.hasAddress()) {
            ...
            fuzzerContext.addLog(new FuzzOperationResult<>("set Address in Practitioner", null, p.hasAddress() ? p.getAddress() : null));
        } else {
    ...
    private void extensionFuzz(Practitioner p) {
        ...
        if (!p.hasExtension()) {
            ...
            fuzzerContext.addLog(new FuzzOperationResult<>("fuz Extension in Practitioner", org, p.hasExtension() ? p.getExtension() : null));
        }
    }
