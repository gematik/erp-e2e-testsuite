...
public class HumanNameFuzzerImpl implements FhirTypeFuzz<HumanName> {
  ...
  public HumanName generateRandom() {
    val hn =
        new HumanName()
            .setUse(
                fuzzerContext.getRandomOneOfClass(HumanName.NameUse.class, HumanName.NameUse.NULL))
    ...
    if (!h.hasUse()) {
      ...
      val newUse =
          fuzzerContext.getRandomOneOfClass(
              HumanName.NameUse.class, List.of(org, HumanName.NameUse.NULL));
      h.setUse(newUse);
    ...
    if (!h.hasText()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "fuzz Text in HumanName:", text, h.hasText() ? h.getText() : null));
    }
  }
  ...
  private void fuzzFamily(HumanName h) {
    if (!h.hasFamily()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "fuzz Family in HumanName:", org, h.hasFamily() ? h.getFamily() : null));
    }
  }
  ...
  private void fuzzGiven(HumanName h) {
    if (!h.hasGiven()) {
      ...
      fuzzerContext
          .getStringFuzz()
          .fuzz(h::getGivenAsSingleString, o -> h.setGiven(List.of(new StringType(o))));
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "fuzz Changes in HumanName:", org, h.hasGiven() ? h.getGivenAsSingleString() : null));
    }
  }
  ...
  private void fuzzPrefix(HumanName h) {
    if (!h.hasPrefix()) {
      ...
      fuzzerContext
          .getStringFuzz()
          .fuzz(h::getPrefixAsSingleString, o -> h.setPrefix(List.of(new StringType(o))));
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "fuzz Prefix in HumanName:",
              org,
              h.hasPrefix() ? h.getPrefixAsSingleString() : null));
    }
  }
  ...
  private void fuzzSuffix(HumanName h) {
    if (!h.hasSuffix()) {
      ...
      fuzzerContext
          .getStringFuzz()
          .fuzz(h::getSuffixAsSingleString, o -> h.setSuffix(List.of(new StringType(o))));
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "fuzz Suffix in HumanName:",
              org,
              h.hasSuffix() ? h.getSuffixAsSingleString() : null));
    }
  }
  ...
  private void fuzzPeriod(HumanName h) {
    ...
    if (!h.hasPeriod()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "Changes Period in HumanName ", null, h.hasPeriod() ? h.getPeriod() : null));
    } else {
      val org = h.getPeriod();
      periodFuzzer.ifPresent(tf -> tf.fuzz(h::getPeriod, h::setPeriod));
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "fuzz Period in HumanName:", org, h.hasSuffix() ? h.getSuffix() : null));
    }
  }
