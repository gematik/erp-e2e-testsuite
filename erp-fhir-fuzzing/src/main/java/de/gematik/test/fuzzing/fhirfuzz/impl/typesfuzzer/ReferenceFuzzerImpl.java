...
public class ReferenceFuzzerImpl implements FhirTypeFuzz<Reference> {
  ...
  private void referenceFuzz(Reference r) {
    ...
    if (!r.hasReference()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>("fuzz reference in Reference:", org, r.getReference()));
    }
  }
  ...
  private void fuzzId(Reference r) {
    ...
    fuzzerContext.addLog(
        new FuzzOperationResult<>("fuzz Id in Reference ", id, r.hasId() ? r.getId() : null));
  }
    ...
    if (!r.hasIdentifier()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "set Identifier in Reference:", null, r.hasIdentifier() ? r.getIdentifier() : null));
    } else {
      val org = r.getIdentifier().copy();
      identifierFuzzer.ifPresent(tf -> tf.fuzz(r::getIdentifier, r::setIdentifier));
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "fuzz Identifier in Reference:", org, r.hasIdentifier() ? r.getIdentifier() : null));
    }
  }
  ...
  private void displayFuzz(Reference r) {
    ...
    if (!r.hasDisplay()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>("set Display in Reference:", disp, r.getDisplay()));
    }
  }
  ...
  private void exceedDisplayLength(Reference r) {
    if (r.hasDisplay()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "set Displaylength up to 50 in Reference:", disp, r.getDisplay()));
    }
  }
  ...
  public Reference generateRandom() {
    ...
    reference
        .setReference(fuzzerContext.getUrlFuzz().generateRandom())
    ...
    fuzzerContext
        ...
        .ifPresent(tf -> reference.setIdentifier(tf.generateRandom()));
    return reference;
  }
}
