...
public class FhirFuzzImpl implements FhirResourceFuzz<Bundle> {
  ...
  private void idFuzz(Bundle b) {
    ...
    fuzzerContext.addLog(
        new FuzzOperationResult<>("set ID in Bundle:", orgId, b.hasId() ? b.getId() : null));
  }
  ...
  private void signatureFuzz(Bundle b) {
    ...
    if (sig == null) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>("set Signature in Bundle:", sig, b.getSignature()));
    } else {
      sigFuzzer.ifPresent(tf -> tf.fuzz(b::hasSignature, b::getSignature, b::setSignature));
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "fuzz Signature in Bundle:", sig, b.hasSignature() ? b.getSignature() : null));
    }
  }
  ...
  private void identifyFuzz(Bundle b) {
    ...
    fuzzerContext
        ...
        .ifPresent(tf -> tf.fuzz(b::hasIdentifier, b::getIdentifier, b::setIdentifier));
    fuzzerContext.addLog(
        new FuzzOperationResult<>(
            "set Identifier in Bundle:", ident, b.hasIdentifier() ? b.getIdentifier() : null));
  }
    ...
    if (!b.hasType()) {
      ...
      val newType =
          fuzzerContext.getRandomOneOfClass(
              Bundle.BundleType.class, List.of(org, Bundle.BundleType.NULL));
      b.setType(newType);
  ...
  private void metaFuzz(Bundle b) {
    ...
    fuzzerContext
        ...
        .ifPresent(tf -> tf.fuzz(b::hasMeta, b::getMeta, b::setMeta));
    fuzzerContext.addLog(
        new FuzzOperationResult<>("set Meta in Bundle:", meta, b.hasMeta() ? b.getMeta() : null));
  }
  ...
  private void langFuzz(Bundle b) {
    if (!b.hasLanguage()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "set Language in Bundle:", lang, b.hasLanguage() ? b.getLanguage() : null));
    }
  }
}
