...
public class CodeableConceptFuzzImpl implements FhirTypeFuzz<CodeableConcept> {
  ...
  private void fuzzText(CodeableConcept cc) {
    if (!cc.hasText()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>("Changes Text in CodeableConcept ", null, txt));
    } else {
      val value = cc.getText();
      fuzzerContext.getStringFuzz().fuzz(cc::getText, cc::setText);
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "Changes Text in CodeableConcept ", value, cc.hasText() ? cc.getText() : null));
    }
  }
  ...
  private void fuzzCoding(CodeableConcept cc) {
    ...
    if (!cc.hasCoding()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "set Coding in CodeableConcept ", null, cc.hasCoding() ? cc.getCoding() : null));
    } else {
      val org = cc.getCoding();
      codingFuzz.ifPresent(tf -> tf.fuzz(cc::getCodingFirstRep, o -> cc.setCoding(List.of(o))));
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "set Coding in CodeableConcept ", org, cc.hasCoding() ? cc.getCoding() : null));
    }
  }
  ...
  private void fuzzExtension(CodeableConcept cc) {
    ...
    if (!cc.hasExtension()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>("set Extension in CodeableConcept ", null, ext));
    } else {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "fuzz Extension in CodeableConcept ",
              ext,
              cc.hasExtension() ? cc.getExtension() : null));
    }
  }
  ...
  private void fuzzId(CodeableConcept cc) {
    if (!cc.hasId()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "fuzz Id in CodeableConcept ", id, cc.hasId() ? cc.getId() : null));
    }
  }

  public CodeableConcept generateRandom() {
    val cc = new CodeableConcept().setText(fuzzerContext.getStringFuzz().generateRandom());
    cc.setId(fuzzerContext.getIdFuzzer().generateRandom());
    fuzzerContext
        .getTypeFuzzerFor(Coding.class)
        .ifPresent(tf -> cc.setCoding(List.of(tf.generateRandom())));
    return cc;
  }
}
