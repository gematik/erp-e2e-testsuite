...
public class SimpleQuantityFuzzImpl implements FhirTypeFuzz<Quantity> {
  ...
  private void valueFuzzer(Quantity q) {
    ...
    fuzzerContext
        .getIntFuzz()
        .fuzz(q::hasValue, () -> q.getValue().toBigInteger().intValue(), q::setValue);
    fuzzerContext.addLog(
        new FuzzOperationResult<>(
            "set Value in SimpleQuantity", org, q.hasValue() ? q.getValue() : null));
  }
  ...
  private void unitFuzzer(Quantity q) {
    ...
    fuzzerContext.addLog(
        new FuzzOperationResult<>(
            "set Unit in SimpleQuantity", org, q.hasUnit() ? q.getUnit() : null));
  }
  ...
  private void systemFuzzer(Quantity q) {
    ...
    fuzzerContext.addLog(
        new FuzzOperationResult<>(
            "set System in SimpleQuantity", org, q.hasSystem() ? q.getSystem() : null));
  }
  ...
  private void codeFuzzer(Quantity q) {
    ...
    fuzzerContext.addLog(
        new FuzzOperationResult<>(
            "set Code in SimpleQuantity", org, q.hasCode() ? q.getCode() : null));
  }
