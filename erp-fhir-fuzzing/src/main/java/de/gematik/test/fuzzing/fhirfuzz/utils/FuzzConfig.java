...
public class FuzzConfig {
    ...
    public static FuzzConfig getRandom() {
        ...
        log.info(format("FuzzConfig called: {6}, Attributes had been setup default with following entries: " +
                ...
                fuzzConf.usedPercentOfMutators, fuzzConf.percentOfAll, fuzzConf.percentOfEach, fuzzConf.useAllMutators, fuzzConf.iterations, fuzzConf.shouldPrintToFile, fuzzConf.name,
                !fuzzConf.detailSetup.isEmpty() ? fuzzConf.detailSetup.entrySet().stream().map(Object::toString).collect(Collectors.joining(", ")) : "no Entry"));
        return fuzzConf;
    }
    ...
    public String toString() {
        return format("FuzzConfig called: {6}: " +
                ...
                this.usedPercentOfMutators, this.percentOfAll, this.percentOfEach, this.useAllMutators, this.iterations, this.shouldPrintToFile, this.name,
                (this.detailSetup != null) ? this.detailSetup.entrySet().stream().map(Object::toString).collect(Collectors.joining(", ")) : "no Entry");
