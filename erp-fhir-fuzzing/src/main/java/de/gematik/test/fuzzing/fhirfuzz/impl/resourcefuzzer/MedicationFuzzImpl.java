...
public class MedicationFuzzImpl implements FhirResourceFuzz<Medication> {
    ...
    private void metaFuzz(Medication m) {
        ...
        fuzzerContext.getTypeFuzzerFor(Meta.class).ifPresent(tf -> tf.fuzz(m::hasMeta, m::getMeta, m::setMeta));
    ...
    private void formFuzz(Medication m) {
        ...
        fuzzerContext.getTypeFuzzerFor(CodeableConcept.class).ifPresent(tf -> tf.fuzz(m::hasForm, m::getForm, m::setForm));
    ...
    private void rangeTexFuzz(Medication m) {
        if (m.hasCode()) {
            m.getCode().setText(fuzzerContext.getStringFuzz().generateRandom(52)); // KBV define a maximum length of 50
        }
    }

    private void amountFuzz(Medication m) {
        val org = m.hasAmount() ? m.getAmount() : null;
        fuzzerContext.getTypeFuzzerFor(Ratio.class).ifPresent(tf -> tf.fuzz(m::hasAmount, m::getAmount, m::setAmount));
    ...
    public Medication generateRandom() {
        ...
        fuzzerContext.getTypeFuzzerFor(Identifier.class).ifPresent(tf -> med.setIdentifier(List.of(tf.generateRandom())));
        ...
        fuzzerContext.getTypeFuzzerFor(CodeableConcept.class).ifPresent(tf -> med.setForm(tf.generateRandom()));
        fuzzerContext.getTypeFuzzerFor(Extension.class).ifPresent(tf -> med.setExtension(List.of(tf.generateRandom())));
        fuzzerContext.getTypeFuzzerFor(CodeableConcept.class).ifPresent(tf -> med.setCode(tf.generateRandom()));
        fuzzerContext.getTypeFuzzerFor(Ratio.class).ifPresent(tf -> med.setAmount(tf.generateRandom()));
        med.setStatus(fuzzerContext.getRandomOneOfClass(Medication.MedicationStatus.class, List.of(Medication.MedicationStatus.NULL)));
        return med;
