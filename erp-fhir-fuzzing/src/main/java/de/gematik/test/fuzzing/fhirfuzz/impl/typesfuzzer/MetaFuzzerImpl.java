...
public class MetaFuzzerImpl implements FhirTypeFuzz<Meta> {
    ...
    private void fuzzLastUpdate(Meta m) {
        ...
        if (!m.hasLastUpdated()) {
            ...
            fuzzerContext.addLog(new FuzzOperationResult<>("fuzz LastUpdate in Meta:",
                    orgDate, newDate));
        }
    }
    ...
    private void fuzzSource(Meta m) {
        if (!m.hasSource()) {
            ...
            fuzzerContext.addLog(new FuzzOperationResult<>("switched Source and VersionId in Meta // original VersionId:", orgVersionId, orgVersionId));
            fuzzerContext.addLog(new FuzzOperationResult<>("switched Source and VersionId in Meta // original Source", orgSource, orgSource));

            if (!m.hasVersionId()) {
                val versionId = fuzzerContext.getUrlFuzz().generateRandom();
                m.setVersionId(versionId);
                fuzzerContext.addLog(new FuzzOperationResult<>("add VersionId after switched Source and VersionId without SourceEntry in Meta", null, versionId));
            }
        }
    }

    private void fuzzProfile(Meta m) {
        val canonicalTypeFuzzer = fuzzerContext.getTypeFuzzerFor(CanonicalType.class, () -> new CanonicalTypeFuzzerImpl(fuzzerContext));
        if (!m.hasProfile()) {
            ...
            fuzzerContext.addLog(new FuzzOperationResult<>("fuzzed Profile  in Meta", prof, m.hasProfile() ? m.getProfile().get(0) : null));
            if (!m.hasProfile()) {
                m.setProfile(List.of(canonicalTypeFuzzer.generateRandom()));
                fuzzerContext.addLog(new FuzzOperationResult<>("set Profile  in Meta for KBV caused be cardinality 1..1", prof, m.hasProfile() ? m.getProfile().get(0) : null));
            }
        }
    }

    private void fuzzSecurity(Meta m) {
        val codingTypeFuzzerImpl = fuzzerContext.getTypeFuzzerFor(Coding.class, () -> new CodingTypeFuzzerImpl(fuzzerContext));
        if (m.getSecurity() == null) {
            ...
            fuzzerContext.addLog(new FuzzOperationResult<>("fuzz Security in Meta ", sec, m.hasSecurity() ? m.getSecurity().get(0) : null));
        }
    }
    ...
    private void fuzzTag(Meta m) {
        ...
        if (m.getTag() == null) {
            ...
            fuzzerContext.addLog(new FuzzOperationResult<>("set Tag in Meta:", null,
                    coding.getSystem() + " " + coding.getVersion() + " " + coding.getCode() + " " + coding.getDisplay() + " " + coding.getUserSelected()));
        } else {
            val sec = m.getTag();
            for (Coding c : sec) {
