...
public class SignatureFuzzImpl implements FhirTypeFuzz<Signature> {
    ...
    private void fuzzData(Signature s) {
        ...
        if (org == null) {
            ...
            if (Boolean.TRUE.equals(fuzzerContext.shouldFuzz(s.getData()))) {
                ...
                fuzzerContext.addLog(new FuzzOperationResult<>("fuzz Data in Signature :", org.toString(), s.hasData() ? s.getData() : null));
            }
        }
    }
    ...
    private void fuzzTargetFormat(Signature s) {
        ...
        fuzzerContext.addLog(new FuzzOperationResult<>("set TargetFormat in Signature :", org, s.hasTargetFormat() ? s.getTargetFormat() : null));
    }
    ...
    private void fuzzSigFormat(Signature s) {
        ...
        fuzzerContext.addLog(new FuzzOperationResult<>("set SigFormat in Signature :", org, s.hasSigFormat() ? s.getSigFormat() : null));
    }
    ...
    private void fuzzType(Signature s) {
        ...
        if (org == null) {
            ...
            fuzzerContext.addLog(new FuzzOperationResult<>("set Type in Signature :", org, s.hasType() ? s.getType() : null));
        } else {
            val typeList = s.getType();
            for (val t : typeList) {
                val orgEntry = t.copy();
                codingTypeFuzz.ifPresent(tf -> tf.fuzz(t));
                fuzzerContext.addLog(new FuzzOperationResult<>("fuzz Type Element in Signature :", orgEntry, t));
            }
        }
    }
    ...
    private void fuzzWhen(Signature s) {
        ...
        if (!s.hasWhen()) {
            ...
            fuzzerContext.addLog(new FuzzOperationResult<>("fuzz When in Signature:",
                    orgDate, newDate));
        }
    }
    ...
    private void fuzzWho(Signature s) {
        ...
        fuzzerContext.addLog(new FuzzOperationResult<>("fuzz Who in Signature :", org, s.hasWho() ? s.getWho() : null));
    }
    ...
    private void fuzzOnBehalfOf(Signature s) {
        ...
        fuzzerContext.addLog(new FuzzOperationResult<>("fuzz onBehalfOf in Signature :", org, s.hasOnBehalfOf() ? s.getOnBehalfOf() : null));
    }

    private void fuzzId(Signature s) {
        val id = s.hasId() ? s.getId() : null;
        fuzzerContext.getIdFuzzer().fuzz(s::hasId, s::getId, s::setId);
        fuzzerContext.addLog(new FuzzOperationResult<>("fuzz Id in Signature ", id, s.hasId() ? s.getId() : null));
    }
    ...
    private void extensionFuzz(Signature s) {
        ...
        if (!s.hasExtension()) {
            ...
            fuzzerContext.addLog(new FuzzOperationResult<>("Extension in Signature", org, s.hasExtension() ? s.getExtension() : null));
        }
    ...
    public Signature generateRandom() {
        ...
        fuzzerContext.getTypeFuzzerFor(Reference.class).ifPresent(tf -> sig.setWho(tf.generateRandom()));
        ...
        fuzzerContext.getTypeFuzzerFor(Extension.class).ifPresent(tf -> sig.setExtension(List.of(tf.generateRandom())));
        return sig;
    }
