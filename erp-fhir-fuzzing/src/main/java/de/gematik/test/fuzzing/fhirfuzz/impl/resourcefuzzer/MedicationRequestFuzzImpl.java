...
public class MedicationRequestFuzzImpl implements FhirResourceFuzz<MedicationRequest> {
  ...
  public MedicationRequest generateRandom() {
    ...
    fuzzerContext
        ...
        .ifPresent(tf -> medReq.setIdentifier(List.of(tf.generateRandom())));
  ...
  private void dosageFuzz(MedicationRequest mr) {
    val dosageFuzz =
        fuzzerContext.getTypeFuzzerFor(Dosage.class, () -> new DosageFuzzImpl(fuzzerContext));
    if (!mr.hasDosageInstruction()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "set Dosage in MedicationRequest:",
              null,
              mr.hasDosageInstruction() ? mr.getDosageInstruction() : null));
    } else {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "fuzz Dosage in MedicationRequest:",
              dosage,
              mr.hasDosageInstruction() ? mr.getDosageInstruction() : null));
    }
  }
  ...
  private void dispRequFuzz(MedicationRequest mr) {
    ...
    if (mRDRC != null) {
      ...
      simpleQuantityFuzz.ifPresent(
          tf -> tf.fuzz(mRDRC::hasQuantity, mRDRC::getQuantity, mRDRC::setQuantity));
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "set DispenseRequest in MedicationRequest:",
              dR,
              mr.getDispenseRequest().hasQuantity()
                  ? mr.getDispenseRequest().getQuantity()
                  : null));
    } else {
      val dispReq = new MedicationRequest.MedicationRequestDispenseRequestComponent();
      fuzzerContext
          .getTypeFuzzerFor(Quantity.class)
          .ifPresent(tf -> dispReq.setQuantity(tf.generateRandom()));
  ...
  private void authoredOnFuzz(MedicationRequest mr) {
    if (!mr.hasAuthoredOn()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>("set AuthoredOn in MedicationRequest:", null, date));
    } else if (fuzzerContext.conditionalChance()) {
      val date = mr.getAuthoredOn();
      mr.setAuthoredOn(null);
      fuzzerContext.addLog(
          new FuzzOperationResult<>("set Date in MedicationRequest:", date, mr.getAuthoredOn()));
    } else {
      val date = mr.getAuthoredOn();
      Date newDate;
      do {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>("fuzz Date in MedicationRequest:", date, newDate));
    }
  }
  ...
  private void medicationFuzz(MedicationRequest mr) {
    ...
    referenceFuzzer.ifPresent(
        tf -> tf.fuzz(mr::hasMedication, mr::getRequester, mr::setMedication));
    fuzzerContext.addLog(
        new FuzzOperationResult<>(
            "fuzz Medication in MedicationRequest:",
            org,
            mr.hasMedication() ? mr.getMedication() : null));
  }
  ...
  private void idFuzz(MedicationRequest mr) {
    ...
    fuzzerContext.addLog(
        new FuzzOperationResult<>(
            "fuzz ID in MedicationRequest:", orgId, mr.hasId() ? mr.getId() : null));
  }
  ...
  private void noteFuzz(MedicationRequest mr) {
    ...
    val annotFuzz =
        fuzzerContext.getTypeFuzzerFor(
            Annotation.class, () -> new AnnotationTypeFuzzImpl(fuzzerContext));
    if (notes == null) {
      val newNote = annotFuzz.generateRandom();
      mr.setNote(List.of(newNote));
      fuzzerContext.addLog(
          new FuzzOperationResult<>("set Note in MedicationRequest", null, newNote));
    } else {
      val org = mr.getNote();
      val listFuzz = new ListFuzzerImpl<>(fuzzerContext, annotFuzz);
      listFuzz.fuzz(mr::getNote, mr::setNote);
      fuzzerContext.addLog(
          new FuzzOperationResult<>("fuzz Note in MedicationRequest", org, mr.getNote()));
    }
  }
  ...
  private void insuranceFuzz(MedicationRequest mr) {
    ...
    if (org == null) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>("set Insurance in MedicationRequest", null, ins));
    } else {
      val listFuzz = new ListFuzzerImpl<>(fuzzerContext, referenceFuzzer);
      listFuzz.fuzz(mr::getInsurance, mr::setInsurance);
      fuzzerContext.addLog(
          new FuzzOperationResult<>("set Insurance in MedicationRequest", org, mr.getInsurance()));
    }
  }
  ...
  private void subjectFuzz(MedicationRequest mr) {
    ...
    fuzzerContext.addLog(
        new FuzzOperationResult<>(
            "set Subject in MedicationRequest", org, mr.hasSubject() ? mr.getSubject() : null));
  }
  ...
  private void requesterFuzz(MedicationRequest mr) {
    ...
    fuzzerContext.addLog(
        new FuzzOperationResult<>(
            "set Requester in MedicationRequest",
            org,
            mr.hasRequester() ? mr.getRequester() : null));
  }
    ...
    if (org != null) {
      newEntry =
          fuzzerContext.getRandomOneOfClass(
              MedicationRequest.MedicationRequestIntent.class,
              List.of(org, MedicationRequest.MedicationRequestIntent.NULL));
    } else {
      newEntry =
          fuzzerContext.getRandomOneOfClass(
              MedicationRequest.MedicationRequestIntent.class,
              List.of(MedicationRequest.MedicationRequestIntent.NULL));
    }
    mr.setIntent(newEntry);
    fuzzerContext.addLog(
        new FuzzOperationResult<>("set Intend in MedicationRequest", org, newEntry));
  }
  ...
  private void statusFuzz(MedicationRequest mr) {
    ...
    if (org != null) {
      newEntry =
          fuzzerContext.getRandomOneOfClass(
              MedicationRequest.MedicationRequestStatus.class,
              List.of(
                  org,
    ...
    fuzzerContext.addLog(
        new FuzzOperationResult<>("set Status in MedicationRequest", org, newEntry));
  }
  ...
  private void langFuzz(MedicationRequest mr) {
    ...
    fuzzerContext.addLog(
        new FuzzOperationResult<>(
            "set Language in MedicationRequest", org, mr.hasLanguage() ? mr.getLanguage() : null));
  }
  ...
  private void identFuzz(MedicationRequest mr) {
    ...
    if (!mr.hasIdentifier()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>("set Identifier in MedicationRequest:", null, ident));
    } else {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "set Identifier in MedicationRequest:",
              ident,
              mr.hasIdentifier() ? mr.getIdentifier() : null));
    }
  }
  ...
  private void metaFuzz(MedicationRequest mr) {
    ...
    fuzzerContext
        ...
        .ifPresent(tf -> tf.fuzz(mr::hasMeta, mr::getMeta, mr::setMeta));
    fuzzerContext.addLog(
        new FuzzOperationResult<>(
            "set Meta in MedicationRequest:", meta, mr.hasMeta() ? mr.getMeta() : null));
  }
  ...
  private void extensionFuzz(MedicationRequest m) {
    ...
    if (!m.hasExtension()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>("set Extension in MedicationRequest", null, ext));
    } else {
      ...
      for (val ex : m.getExtension()) {
        ...
        fuzzerContext.addLog(
            new FuzzOperationResult<>(
                format("set Extension No: {0}in MedicationRequest", count),
                org,
                m.hasExtension() ? m.getExtension() : null));
      }
