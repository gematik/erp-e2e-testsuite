...
public class DateTypeFuzzImpl implements FhirTypeFuzz<DateType> {
  ...
  public DateType generateRandom() {
    return new DateType(fuzzerContext.getRandomDateWithFactor(10));
  }
  ...
  private void idFuzz(DateType d) {
    if (!d.hasId()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>("fuzz Id in DataType:", org, d.hasId() ? d.getId() : null));
    }
  }
  ...
  private void precisionFuzz(DateType d) {
    if (!d.hasTime()) {
      ...
      fuzzerContext.addLog(new FuzzOperationResult<>("set Precision in DataType", pre, newPre));
    }
  }
