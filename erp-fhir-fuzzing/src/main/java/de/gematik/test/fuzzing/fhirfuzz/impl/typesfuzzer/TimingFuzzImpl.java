...
public class TimingFuzzImpl implements FhirTypeFuzz<Timing> {
  ...
  public Timing generateRandom() {
    ...
    fuzzerContext
        ...
        .ifPresent(tf -> t.setCode(tf.generateRandom()));
    fuzzerContext
        .getBaseFuzzerFor(Timing.TimingRepeatComponent.class)
        .ifPresent(tf -> t.setRepeat(tf.generateRandom()));
    return t;
  }
  ...
  private void codeFuzz(Timing t) {
    ...
    if (org == null) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>("set Code in Timing", null, t.hasCode() ? t.getCode() : null));
    } else {
      fuzzerContext
          .getTypeFuzzerFor(CodeableConcept.class)
          .ifPresent(tf -> tf.fuzz(t::hasCode, t::getCode, t::setCode));
      fuzzerContext.addLog(
          new FuzzOperationResult<>("fuzz Code in Timing", org, t.hasCode() ? t.getCode() : null));
    }
  }
  ...
  private void repeatFuzz(Timing t) {
    ...
    fuzzerContext
        .getBaseFuzzerFor(Timing.TimingRepeatComponent.class)
        .ifPresent(tf -> tf.fuzz(t::hasRepeat, t::getRepeat, t::setRepeat));
    fuzzerContext.addLog(
        new FuzzOperationResult<>(
            "set New Repeat in Timing", org, t.hasRepeat() ? t.getRepeat() : null));
  }
