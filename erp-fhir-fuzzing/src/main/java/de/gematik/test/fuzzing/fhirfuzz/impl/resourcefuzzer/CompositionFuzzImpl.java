...
public class CompositionFuzzImpl implements FhirResourceFuzz<Composition> {
  ...
  public Composition generateRandom() {
    ...
    fuzzerContext
        ...
        .ifPresent(tf -> comp.setIdentifier(tf.generateRandom()));
    comp.setStatus(
        fuzzerContext.getRandomOneOfClass(
            Composition.CompositionStatus.class, Composition.CompositionStatus.NULL));
    ...
    if (!c.hasLanguage()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "set Language in Composition", org, c.hasLanguage() ? c.getLanguage() : null));
    }
  }
  ...
  private void identifyFuzz(Composition c) {
    ...
    fuzzerContext
        .getTypeFuzzerFor(Identifier.class)
        .ifPresent(tf -> tf.fuzz(c::hasIdentifier, c::getIdentifier, c::setIdentifier));
    fuzzerContext.addLog(
        new FuzzOperationResult<>(
            "set Identifier in Composition:", ident, c.hasIdentifier() ? c.getIdentifier() : null));
  }
  ...
  private void statusFuzz(Composition c) {
    if (!c.hasStatus()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>("set Status in Composition:", null, compStatus));
    } else {
      val org = c.getStatus();
      val newEntry =
          fuzzerContext.getRandomOneOfClass(
              Composition.CompositionStatus.class,
              List.of(org, Composition.CompositionStatus.NULL));
  ...
  private void metaFuzz(Composition c) {
    ...
    fuzzerContext.addLog(
        new FuzzOperationResult<>(
            "set Meta in Composition:", meta, c.hasMeta() ? c.getMeta() : null));
  }
  ...
  private void typeFuzz(Composition c) {
    ...
    fuzzerContext.addLog(
        new FuzzOperationResult<>(
            "fuzz Type in Composition:", orgCoding, c.hasType() ? c.getType() : null));
  }
  ...
  private void idFuzz(Composition c) {
    ...
    fuzzerContext.addLog(
        new FuzzOperationResult<>("set ID in Composition:", orgId, c.hasId() ? c.getId() : null));
  }
  ...
  private void categoryFuzz(Composition c) {
    ...
    if (cat == null) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>("fuzz Category in Composition:", null, newEntry));
    } else {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "fuzz Category in Composition:",
              orgCoding,
              c.hasCategory() ? c.getCategory() : null));
    }
  }
  ...
  private void subjectFuzz(Composition c) {
    ...
    fuzzerContext
        ...
        .ifPresent(tf -> tf.fuzz(c::hasSubject, c::getSubject, c::setSubject));
    fuzzerContext.addLog(
        new FuzzOperationResult<>(
            "fuzz Subject in Composition:", orgSub, c.hasSubject() ? c.getSubject() : null));
  }
  ...
  private void encounterFuzz(Composition c) {
    ...
    fuzzerContext
        .getTypeFuzzerFor(Reference.class)
        .ifPresent(tf -> tf.fuzz(c::hasEncounter, c::getEncounter, c::setEncounter));
    fuzzerContext.addLog(
        new FuzzOperationResult<>(
            "fuzz Encounter in Composition:", orgSub, c.hasEncounter() ? c.getEncounter() : null));
  }
  ...
  private void dateFuzz(Composition c) {
    if (!c.hasDate()) {
      ...
      c.setDate(null);
      fuzzerContext.addLog(
          new FuzzOperationResult<>("set Date in Composition:", date, c.getDate()));
    } else {
      val date = c.getDate();
      Date newDate;
      do {
  ...
  private void autorsFuzz(Composition c) {
    ...
    if (!c.hasAuthor()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "fuzz Authors in Composition:", org, c.hasAuthor() ? c.getAuthor() : null));
    }
  }
  ...
  private void confidentFuzz(Composition c) {
    if (!c.hasConfidentiality()) {
      val conf =
          fuzzerContext.getRandomOneOfClass(
              Composition.DocumentConfidentiality.class, Composition.DocumentConfidentiality.NULL);
      c.setConfidentiality(conf);
      fuzzerContext.addLog(
          new FuzzOperationResult<>("set Confidentiality in Composition:", null, conf));
    } else {
      val org = c.hasConfidentiality() ? c.getConfidentiality() : null;
      fuzzerContext.getRandomOneOfClass(
          Composition.DocumentConfidentiality.class,
          List.of(org, Composition.DocumentConfidentiality.NULL));
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "fuzz Confidentiality in Composition:",
              org,
              c.hasConfidentiality() ? c.getConfidentiality() : null));
    }
  }
  ...
  private void custodianFuzz(Composition c) {
    ...
    fuzzerContext
        .getTypeFuzzerFor(Reference.class)
        .ifPresent(tf -> tf.fuzz(c::hasCustodian, c::getCustodian, c::setCustodian));
    fuzzerContext.addLog(
        new FuzzOperationResult<>(
            "fuzz Custodian in Composition:",
            ref,
            c.hasCustodian() ? c.getCustodian().getReference() : null));
  }
  ...
  private void relatesToFuzz(Composition c) {
    ...
    if (!c.hasRelatesTo()) {
      ...
      relTo.setCode(
          fuzzerContext.getRandomOneOfClass(
              Composition.DocumentRelationshipType.class,
              Composition.DocumentRelationshipType.NULL));
      fhirIdentifierFuzzer.ifPresent(tf -> relTo.setTarget(tf.generateRandom()));
      c.setRelatesTo(List.of(relTo));
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "set RelatesTo in Composition:", null, relTo.getTarget().getId()));
    } else {
      val orgRelTo = c.getRelatesToFirstRep().copy();
      fhirIdentifierFuzzer.ifPresent(tf -> tf.fuzz(c::getIdentifier, c::setIdentifier));
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "fuzz RelatesTo in Composition:",
              orgRelTo,
              c.hasRelatesTo() ? c.getRelatesToFirstRep() : null));
    }
  }
  ...
  private void extensionFuzz(Composition c) {
    ...
    if (!c.hasExtension()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "Extension in Composition", org, c.hasExtension() ? c.getExtension() : null));
    }
  }
}
