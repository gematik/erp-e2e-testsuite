...
public class FuzzerFactory {
  ...
  public <T extends Type> Optional<FhirTypeFuzz<T>> getTypeFuzzerFor(Class<T> xClass) {
    ...
    if (listOfClasFuzzer.size() < 1) {
      log.info(
          "You tried to fuzz an element without adding a Fuzzer to the List of ClassTypeFuzzer! Now"
              + " you got an Optional.empty()");
  ...
  public <T extends Type> FhirTypeFuzz<T> getTypeFuzzerFor(
      Class<T> xClass, Supplier<FhirTypeFuzz<T>> defaultValue) {
  ...
  public <R extends Resource> Optional<FhirResourceFuzz<R>> getFuzzerFor(Class<R> xClass) {
    ...
    if (listOfClasFuzzer.isEmpty()) {
      log.info(
          "You tried to fuzz an element without adding a Fuzzer to the List of ClassResourceFuzzer!"
              + " Now you got an Optional.empty()");
      return Optional.empty();
    }
    val idx = fuzzerContext.getRandom().nextInt(listOfClasFuzzer.size());
    return Optional.of(listOfClasFuzzer.get(idx));
  ...
  public <B extends Base> Optional<BaseFuzzer<B>> getBaseFuzzerFor(Class<B> bClass) {
    ...
    if (listOfClasFuzzer.isEmpty()) {
      log.info(
          "You tried to fuzz an element without adding a Fuzzer to the List of ClassBaseFuzzer! Now"
              + " you got an Optional.empty()");
      return Optional.empty();
    }
    val idx = fuzzerContext.getRandom().nextInt(listOfClasFuzzer.size());
