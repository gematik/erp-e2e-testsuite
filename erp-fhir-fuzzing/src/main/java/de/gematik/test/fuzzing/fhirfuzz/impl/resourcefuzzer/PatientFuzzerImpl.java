...
public class PatientFuzzerImpl implements FhirResourceFuzz<Patient> {
  ...
  private void idFuzz(Patient p) {
    ...
    fuzzerContext.addLog(
        new FuzzOperationResult<>("fuzzed Id in Patient", id, p.hasId() ? p.getId() : null));
  }
  ...
  private void metaFuzz(Patient p) {
    ...
    fuzzerContext.addLog(
        new FuzzOperationResult<>("fuzz Meta in Patient", meta, p.hasMeta() ? p.getMeta() : null));
  }
  ...
  private void langFuzz(Patient p) {
    ...
    fuzzerContext.addLog(
        new FuzzOperationResult<>(
            "set Language in Patient", org, p.hasLanguage() ? p.getLanguage() : null));
  }
  ...
  private void textFuzz(Patient p) {
    ...
    fuzzerContext.addLog(
        new FuzzOperationResult<>("fuzz Text in Patient", org, p.hasText() ? p.getText() : null));
  }
  ...
  private void nameFuzz(Patient p) {
    ...
    if (!p.hasName()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>("set HumanName 1st Entry in Patient", null, hName));
    } else {
  ...
  private void extensionFuzz(Patient p) {
    ...
    if (!p.hasExtension()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>("set Extension in Patient", null, ex.getValue()));
    } else {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "fuzz Extension in Patient",
              org.get(0).getValue(),
              p.hasExtension() ? p.getExtension().get(0).getValue() : null));
    }
  }
  ...
  private void addressFuzz(Patient p) {
    ...
    if (!p.hasAddress()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "fuzz Address in Patient", org, p.hasAddress() ? p.getAddress() : null));
    }
  }
  ...
  private void codeConFuzz(Patient p) {
    ...
    if (!p.hasMaritalStatus()) {
      ...
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "set MaritalStatus in Patient",
              null,
              p.hasMaritalStatus() ? p.getMaritalStatus() : null));
    } else {
      codeableCon.ifPresent(tf -> tf.fuzz(p::getMaritalStatus, p::setMaritalStatus));
      fuzzerContext.addLog(
          new FuzzOperationResult<>(
              "fuzz MaritalStatus in Patient",
              null,
              p.hasMaritalStatus() ? p.getMaritalStatus() : null));
    }
  }
  ...
  public Patient generateRandom() {
    ...
    fuzzerContext
        ...
        .ifPresent(tf -> p.setAddress(List.of(tf.generateRandom())));
    fuzzerContext
        .getTypeFuzzerFor(HumanName.class)
        .ifPresent(tf -> p.setName(List.of(tf.generateRandom())));
