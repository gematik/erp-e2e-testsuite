...
Funktionalität: Demo Feature für PKV Tests
  ...
  Grundlage:
    ...
    Und der Versicherte hat seine Einwilligung zum Speichern der PKV-Abrechnungsinformationen widerrufen
    Und die PKV Versicherte Hanna Bäcker hat Zugriff auf ihre eGK
    Und die Versicherte hat ihre Einwilligung zum Speichern der PKV-Abrechnungsinformationen erteilt

  @TCID:ERP_EE_PKV_DEMO_01
  @TESTFALL:positiv
  ...
  der Vorbedingung
    Wenn der Versicherte Fridolin Straßer seine Einwilligung zur Speicherung der PKV-Abrechnungsinformationen erteilt
    Dann hat der Versicherte eine Einwilligung zur Speicherung der PKV-Abrechnungsinformationen beim E-Rezept Fachdienst
    Und die Versicherte Hanna Bäcker hat eine Einwilligung zur Speicherung der PKV-Abrechnungsinformationen beim E-Rezept Fachdienst

  @TCID:ERP_EE_PKV_DEMO_02
  @TESTFALL:positiv
  @Hauptdarsteller:Versicherter
  ...
  Die Apotheke dispensiert ein Medikament an einen PKV-Versicherten, der seine Einwilligung erteilt hat
    ...
    Und der Versicherte hat die PKV-Abrechnungsinformationen für das letzte dispensierte Medikament beim Fachdienst vorliegen
