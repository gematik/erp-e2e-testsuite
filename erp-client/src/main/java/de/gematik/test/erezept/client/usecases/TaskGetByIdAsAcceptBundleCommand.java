...
public class TaskGetByIdAsAcceptBundleCommand extends BaseCommand<ErxAcceptBundle> {
  ...
  public TaskGetByIdAsAcceptBundleCommand(TaskId taskId, Secret secret) {
    this(taskId, null, secret);
  }

  private TaskGetByIdAsAcceptBundleCommand(
      TaskId taskId, @Nullable AccessCode accessCode, @Nullable Secret secret) {
    super(ErxAcceptBundle.class, HttpRequestMethod.GET, "Task", taskId.getValue());
