...
public class RejectUseCase {
  ...
  public static Response rejectPrescription(
    ...
    return rejectPrescription(actor, taskId, new AccessCode(accessCode), new Secret(secret));
  }

  public static Response rejectPrescription(
      Pharmacy actor, String taskId, AccessCode accessCode, Secret secret) {
    val rejectCommand = new TaskRejectCommand(taskId, accessCode, secret);
    val rejectResponse = actor.erpRequest(rejectCommand);
    if (rejectResponse.isOperationOutcome() || rejectResponse.getStatusCode() > 299) {
      throw new WebApplicationException(
          Response.status(rejectResponse.getStatusCode())
              .entity(new ErrorResponse(rejectResponse))
              .build());
    }
