...
pipeline {
    ...
    stages {
        stage('Start Service') {
            ...
            steps {
                echo "Start Service"
                dockerRun(IMAGE_NAME, params.Version, SERVICE_NAME, "-ti -p6095:9095 --restart always -eCONF=ibm_tu_config.yaml -eHTTP_PROXY=${http_proxy} -eHTTPS_PROXY=${http_proxy} -eNO_PROXY=${no_proxy}")
            }
        }
