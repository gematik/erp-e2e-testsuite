...
public interface ICommunicationType<T extends ProfileVersion<?>> {
  ...
  INamingSystem getRecipientNamingSystem();

  INamingSystem getSenderNamingSystem();

  default Reference getRecipientReference(@NonNull String value) {
  ...
  default Reference getSenderReference(@NonNull String value) {
  ...
  static <V extends ProfileVersion<?>> ICommunicationType<V> fromUrl(
    ...
    val ret =
        Stream.concat(
            ...
            .filter(type -> type.getTypeUrl().contains(unversionedUrl))
            .findFirst()
