...
public class ErxChargeItemBundle extends Bundle {
  ...
  public DavAbgabedatenBundle getAbgabedatenBundle() {
    return this.getEntry().stream()
        .map(BundleEntryComponent::getResource)
        .filter(resource -> resource.getResourceType().equals(ResourceType.Bundle))
        .filter(resource -> AbdaErpPkvStructDef.PKV_ABGABEDATENSATZ.match(resource.getMeta()))
        .map(DavAbgabedatenBundle.class::cast)
        .findFirst()
        .orElseThrow(
            () ->
