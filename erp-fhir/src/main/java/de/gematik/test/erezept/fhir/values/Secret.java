...
public class Secret extends SemanticValue<String, ErpWorkflowNamingSystem> {

  private Secret(String secret) {
    super(ErpWorkflowNamingSystem.SECRET, secret);
  }

  public static Secret from(Identifier identifier) {
    return Optional.of(identifier)
        .filter(ErpWorkflowNamingSystem.SECRET::matches)
        .map(id -> Secret.from(id.getValue()))
        .orElseThrow(
            () ->
                new BuilderException(
                    format("Cannot extract Secret from {0}", identifier.getSystem())));
  }

  public static Secret from(String secret) {
    return new Secret(secret);
  }

  public static Secret random() {
    return from(GemFaker.fakerSecret());
  }
}
