...
public class ConfigurationFactory<DTO extends BaseConfigurationDto> {
  ...
  private ConfigurationFactory(Builder<DTO> builder) {
   ...
   */
  public static Builder<PrimsysConfigurationBase> forPrimSysConfiguration() {
    return forConfiguration(
        PrimsysConfigurationBase.class, TestsuiteconfigurationScope.ERP_PRIMSYS);
  }

  private static <BDTO extends BaseConfigurationDto> Builder<BDTO> forConfiguration(
      Class<BDTO> type, ConfigurationScope scope) {

    // calculate a default path
    ...
    return new Builder<>(type, configPath, scope);
  }
  ...
  public static class Builder<BDTO extends BaseConfigurationDto> {
    ...
    public BDTO create(boolean withPcs) {
      val factory = new ConfigurationFactory<>(this);
      return factory.create(withPcs);
    }

    public <T extends BaseConfigurationWrapper> T wrappedBy(
        Function<BDTO, T> constructor, boolean withPcs) {
      return constructor.apply(this.create(withPcs));
    }

    public <T extends BaseConfigurationWrapper> T wrappedBy(Function<BDTO, T> constructor) {
