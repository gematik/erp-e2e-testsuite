...
import de.gematik.test.konnektor.cfg.KonnektorContextConfiguration;
...
class CardTerminalManagerTest {
  ...
  void setUp() {
    ...
    Konnektor konnektor =
        new KonnektorImpl(
            context.getContextType(), "KONSIM", KonnektorType.REMOTE, mockProvider, cats1Client);
  ...
  void softKonnShouldAlsoWork() {
    val konnektor =
        new KonnektorImpl(context.getContextType(), "KONSIM", KonnektorType.LOCAL, mockProvider);
  ...
  void noCardTerminalClientsAvailable() {
    val konnektor =
        new KonnektorImpl(context.getContextType(), "KONSIM", KonnektorType.REMOTE, mockProvider);
    assertDoesNotThrow(() -> konnektor.getCardTerminalManager().refresh());
    val egk = smartcards.getEgkByICCSN("80276883110000113298");
    assertDoesNotThrow(() -> konnektor.getCardTerminalManager().insertCard(egk));
  }
