...
class PrescriptionDetailsTests {
  ...
  void checkDeleteButton() {
    ...
    assertEquals(
        element.getIosLocator().get(), By.xpath("(//XCUIElementTypeAlert[@name='Das Rezept ist gerade in Bearbeitung durch eine Apotheke und kann nicht gelöscht werden.'])"));
  ...
  void checkDeletePrescriptionItemButton() {
    ...
    assertTrue(element.getFullName().contains("Delete Prescription button"));
    assertTrue(element.name().toLowerCase().contains("button"));
    assertEquals(element.getIosLocator().get(), AppiumBy.xpath("//*[@name='Horizontaler Rollbalken, 1 Seite']"));
    assertNull(element.getAndroidLocator().get());
  }
