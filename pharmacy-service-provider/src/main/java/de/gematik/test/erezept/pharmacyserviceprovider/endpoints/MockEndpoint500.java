...
public class MockEndpoint500 {

  private static final int RESPONS_CODE = 500;

  @POST
  @Path("{ti_id}")
  public Response versOnly(
      @PathParam("ti_id") String telemId, @QueryParam("req") String transactionId, byte[] body) {
    return ResponsesCheck.generateResponse(telemId, transactionId, body, RESPONS_CODE);
  }

  @POST
  public Response decodeVersOnlySecondId(@QueryParam("req") String transactionId, byte[] body) {
    return ResponsesCheck.generateResponse(transactionId, body, RESPONS_CODE);
  }
}
