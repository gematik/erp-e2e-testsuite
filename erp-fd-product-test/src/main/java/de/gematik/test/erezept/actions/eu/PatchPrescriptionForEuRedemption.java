...
public class PatchPrescriptionForEuRedemption extends ErpAction<ErxTask> {
  ...
  public ErpInteraction<ErxTask> answeredBy(Actor actor) {
    ...
    val parameters = EuPatchTaskInputBuilder.builder().build();

    val cmd = new TaskPatchCommand(taskId, parameters);
    return performCommandAs(cmd, patient);
  }

  public static PatchPrescriptionForEuRedemption of(TaskId taskId) {
    return new PatchPrescriptionForEuRedemption(taskId);
  }
}
