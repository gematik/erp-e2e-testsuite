...
class DispensePrescriptionOldTest extends ErpFhirBuildingTest {
  ...
  void shouldPerformDispensationCorrectWithMedDsp() {
    ...
    val action =
        DispensePrescriptionOld.withCredentials(taskId, secret)
    ...
    when(useErpClient.request(any(DispensePrescriptionAsBundleCommandOld.class)))
        .thenReturn(response);
  ...
  void shouldPerformDispensationCorrectWithParameters() {
    ...
    val action = DispensePrescriptionOld.withCredentials(taskId, secret).withParameters(params);
