...
class GetEuMedicationDispensesTest extends ErpFhirBuildingTest {
  ...
  void shouldGetMedicationDispenseWhenPrepared() {
    assertDoesNotThrow(
        () ->
            sina.performs(GetEuMedicationDispenses.whenPrepared(SearchPrefix.EQ, LocalDate.now())));
  }
  ...
  void shouldGetMedicationDispenseFromPerformer() {
    assertDoesNotThrow(
        () ->
            sina.performs(GetEuMedicationDispenses.fromPerformer(TelematikID.from("performerId"))));
  }

  @Test
  void shouldGetMedicationDispenseWithQuerry() {
    assertDoesNotThrow(
        () ->
            sina.performs(
                GetEuMedicationDispenses.withQueryParams(
                    IQueryParameter.search().hasSender("id").createParameter())));
  }

  @Test
  void shouldGetMedicationDispenseForPrescription() {
    assertDoesNotThrow(
        () -> sina.performs(GetEuMedicationDispenses.forPrescription(PrescriptionId.random())));
  }
            ...
            sina.performs(
                GetMedicationDispense.withQueryParams(
                    IQueryParameter.search()
                        .whenHandedOver(SearchPrefix.EQ, LocalDate.now())
                        .fromPerformer(TelematikID.from("performerId"))
                        .whenPrepared(SearchPrefix.EQ, LocalDate.now())
                        .identifierValue(PrescriptionId.random().asIdentifier())
                        .createParameter())));
  }
}
